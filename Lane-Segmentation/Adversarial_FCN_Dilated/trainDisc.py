...
def train_wgan_gp():
    ...
    for EPOCHS in range(100):
        ...
        if (running_loss / g_updates) < best_loss:
            best_loss = running_loss/g_updates
            best_loss_epoch = EPOCHS
            torch.save(netG.state_dict(), 'best_loss_wgangp.wts')
        if (running_loss_cr / g_updates) < best_loss_cr:
            best_loss_cr = running_loss_cr / g_updates
            best_loss_cr_ep = EPOCHS
            torch.save(netG.state_dict(), 'best_seg_wgangp.wts')

        print("Epoch %d: LossG = %f ErrGadv = %f ErrG_cr = %f LossD = %f Best_Loss = %f Best_Loss_Epoch = %d" % (EPOCHS, running_loss/g_updates, running_adv/g_updates, running_loss_cr/g_updates, errD, best_loss, best_loss_epoch))    # lr_decayer(optimizer, EPOCHS)
        print('Best Seg iter', best_loss_cr_ep, 'Best Seg Loss', best_loss_cr)
        lr_decayer(optimizerG, optimizerD, EPOCHS)
